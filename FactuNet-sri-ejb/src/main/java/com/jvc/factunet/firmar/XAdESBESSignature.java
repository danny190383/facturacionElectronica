...
//    signature.setPathSignature(pathSignature);
  ...
  public static void firmar(String xmlPath, InputStream pathSignature, String passSignature, String pathOut, String nameFileOut)
    throws CertificateException, IOException
  {
    XAdESBESSignature signature = new XAdESBESSignature(xmlPath);
    signature.setPassSignature(passSignature);
    signature.setFirmaElectronica(pathSignature); 
    pathFile = pathOut;
    nameFile = nameFileOut;
    signature.execute();
  }
